 ...
 */
class Term extends REST_Controller {
    ...
    {
        $search_query = '`term_trashed_at` IS NULL AND `term_status` = 2';        
        $data['rows'] = $this->Base_model->list_all('tz_term', '', '', 'term_name', 'asc', $search_query);
        $this->set_response($data, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
    }

    public function index_get()
    {
        $search_query = '`term_trashed_at` IS NULL';
        
        $search = $this->get('q', TRUE);        
        if(!empty($search))
        {
            $search_query .= ' AND (`term_name` LIKE "%'.$search.'%")';
        }
        $total_segments = $this->uri->total_segments();
        $total_count = $this->Base_model->count('tz_term', $search_query);
        ...
        $sort = (!empty($sort)) ? $sort : 'term_created_at';
        $order = $this->input->get('order', TRUE);
        $rows = $this->Base_model->list_all('tz_term', $pagination['per_page'], $this->uri->segment($total_segments), $sort, $order, $search_query);
        if(is_array($rows))
        {
            foreach($rows as $key => $val)
            {
                $data['rows'][$key] = $val;
                $data['rows'][$key]['term_created_at'] = dateformat($val['term_created_at'], 'Y/m/d');
            }
        }
    ...
    public function detail_get()
    {
        ...
        {
            $row = $this->Base_model->list_all('tz_term', '', '', '', '', 'term_id="'.$id.'"');
            if(is_array($row[0]))
            {
        ...
        if(sizeof($_POST) > 0)
        {
            ...
            if($this->form_validation->run() == true)
            {
                if(!empty($id) && is_numeric($id))
                {
                    ...
                    $post_data['term_modified_at'] = datenow();
                    $row = $this->Base_model->update($post_data, $cond, 'tz_term');
                    if(isset($row['term_id']) && !empty($row['term_id']))
                    {
                        $message['term_id'] = $row['term_id'];
                ...
                {
                    $post_data['term_created_at'] = datenow();
                    $post_data['term_modified_at'] = datenow();
                    $id = $this->Base_model->insert($post_data, 'tz_term');
                    if(!empty($id))
                    {
                        $message['term_id'] = $id;
                        $message['status'] = 'success';
    ...
    public function delete_get()
    {
        ...
        {
            $cond = array('term_id' => $id);
            $post_data['term_trashed_at'] = datenow();
            $id = $this->Base_model->update($post_data, $cond, 'tz_term');
            if(!empty($id))
            {
                $message['status'] = 'success';
