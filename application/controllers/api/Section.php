...
class Section extends REST_Controller {
    ...
    {
        $search_query = '`section_trashed_at` IS NULL AND `section_status` = 2';        
        $data['rows'] = $this->Base_model->list_all('tz_section', '', '', 'section_name', 'asc', $search_query);
        $this->set_response($data, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
    }

    public function index_get()
    {
        $search_query = '`section_trashed_at` IS NULL';
        
        $search = $this->get('q', TRUE);        
        if(!empty($search))
        {
            $search_query .= ' AND (`section_name` LIKE "%'.$search.'%")';
        }
        $total_segments = $this->uri->total_segments();
        $total_count = $this->Base_model->count('tz_section', $search_query);
        ...
        $sort = (!empty($sort)) ? $sort : 'section_created_at';
        $order = $this->input->get('order', TRUE);
        $rows = $this->Base_model->list_all('tz_section', $pagination['per_page'], $this->uri->segment($total_segments), $sort, $order, $search_query);
        if(is_array($rows))
        {
            foreach($rows as $key => $val)
            {
                $data['rows'][$key] = $val;
                $data['rows'][$key]['section_created_at'] = dateformat($val['section_created_at'], 'Y/m/d');
            }
        }
    ...
    public function detail_get()
    {
        ...
        {
            $row = $this->Base_model->list_all('tz_section', '', '', '', '', 'section_id="'.$id.'"');
            if(is_array($row[0]))
            {
    ...
    public function edit_post()
    {
        ...
        if(sizeof($_POST) > 0)
        {
            ...
            if($this->form_validation->run() == true)
            {
                ...
                {
                    $cond = array('section_id' => $id);
                    $post_data['section_modified_at'] = datenow();
                    $row = $this->Base_model->update($post_data, $cond, 'tz_section');
                    if(isset($row['section_id']) && !empty($row['section_id']))
                    {
                        $message['section_id'] = $row['section_id'];
                ...
                {
                    // $post_data['section_current_status_id'] = 2;
                    $post_data['section_created_at'] = datenow();
                    $post_data['section_modified_at'] = datenow();
                    $id = $this->Base_model->insert($post_data, 'tz_section');
                    if(!empty($id))
                    {
                        $message['section_id'] = $id;
                        $message['status'] = 'success';
                        $message['alert'] = 'Data Successfully Saved';
                    }
                    else
    ...
    public function delete_get()
    {
        ...
        {
            $cond = array('section_id' => $id);
            $post_data['section_trashed_at'] = datenow();
            $id = $this->Base_model->update($post_data, $cond, 'tz_section');
            if(!empty($id))
            {
                $message['status'] = 'success';
